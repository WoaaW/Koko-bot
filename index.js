...
let blacklist = []
let CLASSEMENT = []
let POURCENTAGE = [75,16.4,5,1.5,0.1,0.01,1.99]
let READABLEPOURCENTAGE = []
...
const baseRandomTacosTier6 = [
  "https://imgur.com/a/JWEWinC",
  "https://imgur.com/L1s1VAU"
]
...
const baseRandomTacosTierMaudit = [
  ...
  "https://i.pinimg.com/564x/0d/7d/78/0d7d7898e168c4bb6474a8817df31f8d--ketchup-tacos.jpg",
  "https://cdn.discordapp.com/attachments/592830953202909194/996860733914628126/unknown.png"
]
...
db.set("randomTacosTier6", baseRandomTacosTier6);
...
async function addMessageMax(name) {
...
async function checkIfBlacklisted(name) {
  db.get("messageMax").then(messageMax => {
    persIndex = checkForIndex(messageMax, name) + 1;
...
function pourcentage(totalOdds, length){
  let a = 0
  READABLEPOURCENTAGE = [];
  for (i = 0; i < POURCENTAGE.length; i++) {
      a = a + POURCENTAGE[i];
    }
    if (a != 100 || length != POURCENTAGE.length){
      throw Error("Pourcentage erroné")
    }
    for (i = 0; i < POURCENTAGE.length; i++){
      POURCENTAGE[i] = POURCENTAGE[i] * (totalOdds / 100);
      if(i != 0){
        POURCENTAGE[i] = POURCENTAGE[i] + POURCENTAGE[i - 1];      
      }
    }
    for (i = 0; i < POURCENTAGE.length; i++) {
      READABLEPOURCENTAGE.push(POURCENTAGE[i]);    
        }
}

pourcentage(10000,7);
console.log("Odds : " + READABLEPOURCENTAGE );
...
Client.on("messageCreate", message => {
  ...
  if (message.content === prefix + "rule") {
  ...
  else if (message.content === prefix + "Odds") {
    ...
    if (slowMode === 0) {
      ...
      checkIfBlacklisted(message.author.username).then((response) => {
        flag = response;
        ...
        if (flag === false) {
          const randTier = Math.floor(Math.random() * 10000);
          if (randTier < READABLEPOURCENTAGE[0]) {
            db.get("messageMax").then(messageMax => {
          ...
          else if (randTier < READABLEPOURCENTAGE[1]) {
            db.get("messageMax").then(messageMax => {
              addMessage(message.author.username)
              checkIfBlacklisted(message.author.username)     
          ...
          else if (randTier < READABLEPOURCENTAGE[2]) {
            db.get("messageMax").then(messageMax => {
              addMessage(message.author.username)
              checkIfBlacklisted(message.author.username)     
          ...
          else if (randTier < READABLEPOURCENTAGE[3]) {
            db.get("messageMax").then(messageMax => {
              addMessage(message.author.username)
              checkIfBlacklisted(message.author.username)     
          ...
          else if (randTier < READABLEPOURCENTAGE[4]) {
            db.get("messageMax").then(messageMax => {
              addMessage(message.author.username)
              checkIfBlacklisted(message.author.username)     
          ...
          else if (randTier < READABLEPOURCENTAGE[5]) {
            db.get("messageMax").then(messageMax => {
              addMessage(message.author.username)
              checkIfBlacklisted(message.author.username)     
        
            })
            db.get("randomTacosTier6").then(randomTacosTier6 => {
              const connoisseur = Math.floor(Math.random() * randomTacosTier6.length);
              const embed = new Discord.MessageEmbed()
                .setColor("#f20505")
                .setTitle("Tacos Tier 6 ! Légendaire doré !")
                .setImage(randomTacosTier6[connoisseur])
                .setDescription("+ 30 000 xp");
              message.reply({ embeds: [embed] });
            })
            db.get("starterXp").then(starterXp => {
              if (checkForDuplicates(starterXp, message.author.username) === true) {
              ...
              addXp(30000, message.author.username);

            })
          }
          else if (randTier < READABLEPOURCENTAGE[6]) {
            db.get("messageMax").then(messageMax => {
              addMessage(message.author.username)
              checkIfBlacklisted(message.author.username)     
